...
| Network transmission of keystrokes/clipboard without opt-in | **Clean** | Core engines (`core/suggestion`, `core/transliteration`, `core/spellcheck`) are 100% offline (bundled JSONs). Only online path is `translate/TranslationService.java` POST to `https://api.babeltech.com/translate` and Google Fonts HTTPS download — both gated by `PreferencesRepository.isOnlineTranslationEnabled()` / `FontRepository` opt-in and `NetworkPolicy.requireHttps`. Default OFF. |
...
| ML Kit Smart Reply (Phase 3, not yet active) | `suggestion/SmartReplyManager.java` + `core/smartreply/SmartReplyEngine` stub | Gated `PreferencesRepository.isAiRepliesEnabled()` — default OFF. Phase 1/2 stub returns empty, no network. Future online fallback would POST to a provider (to be named). | — | — | — | **Yes, default OFF** (stub) |
| GIF/sticker search (Phase 3, not yet implemented) | — | — | — | — | — | Not present in Phase 1/2 build — no SDK, no call |

**No other network calls** exist in the codebase (verified via `grep -R "http://\|https://\|HttpURLConnection\|OkHttpClient\|Retrofit\|fetch("` — only the two URLs above).

**Data minimization:** Online translation sends only the sentence the user explicitly requests to translate, never the entire clipboard or keystroke buffer. Font download sends only font name.
...
| 5 | `Emoji codePoints` are raw Unicode — no risk, but future Noto Color Emoji font file not yet licensed. | **Info** | Not a risk in Phase 2 scaffold (no file bundled). When added, append SIL OFL/Apache 2.0 notice per font to `LICENSE`/`NOTICE`. | Legal |
